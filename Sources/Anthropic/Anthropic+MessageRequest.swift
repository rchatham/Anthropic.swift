...
public extension Anthropic {
    struct MessageRequest: StreamableLangToolRequest, CompletableLangToolRequest, Codable {
        ...
        public func completion(response: Anthropic.MessageResponse) throws -> Anthropic.MessageRequest? {
            ...
            var toolResults: [Anthropic.Content.ContentType] = []
            for tool_use in array.compactMap({ if case .toolUse(let tool_use) = $0 { return tool_use } else { return nil }}) {
                if let tool = tools?.first(where: { $0.name == tool_use.name }) {
                    ...
                    toolResults.append(.toolResult(.init(tool_use_id: tool_use.id, is_error: false, content: .string(str))))
                }
            }
...
extension Anthropic.MessageResponse: StreamableLangToolResponse {
